...
class EventLocationService {
  ...
  static const Duration _eventLocationResolveDelay = Duration(
    milliseconds: 1300,
  );
  static const Duration _photoLocationResolveDelay = Duration(
    milliseconds: 450,
  );
  ...
  Future<void> resolveEventLocations({
    ...
    while (true) {
      ...
      for (final event in events) {
        await _resolveSingleEventLocation(isar: isar, event: event);
        await Future.delayed(_eventLocationResolveDelay);
      }
    }
  }
  ...
  Future<void> resolvePhotoLocationsForVisibleEvents({
    ...
    while (true) {
      ...
      for (final photo in photos) {
        ...
        await _resolveSinglePhotoLocation(isar: isar, photo: photo);
        await Future.delayed(_photoLocationResolveDelay);
      }
    }
  }
